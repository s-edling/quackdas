...
- 2026-03-09: In the Firefox extension, sidebar-triggered screenshot capture cannot rely on the transient `activeTab` grant. For reliable `tabs.executeScript(...)` plus `captureVisibleTab(...)`, request optional `<all_urls>` access from the sidebar click path and let shortcuts only check whether that grant already exists.
