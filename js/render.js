...
function renderCurrentDocumentView() {
    renderCurrentDocument();
    finalizeRenderCycle();
}

function renderDocumentsAndCurrentDocument() {
    renderDocuments();
    renderCurrentDocument();
    finalizeRenderCycle();
}

function renderCodesAndCurrentDocument() {
    renderCodes();
    renderCurrentDocument();
    finalizeRenderCycle();
}

function renderCasesAndCurrentDocument() {
    if (typeof renderCases === 'function') renderCases();
    renderCurrentDocument();
    finalizeRenderCycle();
}

function renderDocumentsCodesAndCurrentDocument() {
    renderDocuments();
    renderCodes();
    renderCurrentDocument();
    finalizeRenderCycle();
...
function renderCurrentDocument() {
    ...
    if (!appData.filterCodeId) {
        ...
        if (codeInspectorState.segmentId && codeInspectorState.docId && codeInspectorState.docId !== appData.currentDocId) {
            codeInspectorState.segmentId = null;
            codeInspectorState.docId = null;
            codeInspectorState.selectedCodeId = '';
        }
        renderCodeInspector();
    }
...
function renderCodeInspector() {
    ...
    const isCodeViewInspector = !!(appData.filterCodeId && codeViewUiState.mode === 'segments' && codeInspectorState.segmentId);
    const isDocumentInspector = !!(
        codeInspectorState.segmentId &&
        segment &&
        appData.currentDocId === segment.docId &&
        !appData.filterCodeId
    );
    const isVisible = !!(segment && (isCodeViewInspector || isDocumentInspector));
...
function openCodingInspectorForSegment(segmentId, preferredCodeId = '') {
    ...
    codeInspectorState.selectedCodeId = targetCodeId;

    if (appData.currentDocId !== segment.docId) {
        appData.filterCodeId = null;
        selectDocument(segment.docId);
        return;
    }
