...
## Version 0.6.6-build-2026-02-19do
Date: 2026-02-19

What changed:
- Improved QDPX code-color parsing to support more external/NVivo color encodings instead of defaulting to gray.
- Added parsing support for:
  - signed/unsigned integer ARGB values,
  - `0x`-prefixed hex values,
  - `rgb(...)`/`rgba(...)`,
  - comma-separated `R,G,B` and `A,R,G,B`.
- Kept existing ARGB hex handling (`AARRGGBB` -> `RRGGBB`) and fallback to gray only for truly invalid formats.

Why (one line):
- To preserve imported NVivo code colors correctly instead of showing gray for non-plain-hex color formats.
...
What changed:
- Fixed top header layout regression where the global search bar overlapped/shifted near the logo:
  - moved header to a 3-column grid (`left / centered search / right`),
  - centered the search bar in the window,
  - scoped document-toolbar `doc-actions` alignment rules to `.content-header` so they no longer affect the top header.
- Further expanded NVivo/QDPX code color import compatibility:
  - reads color from alternate code attributes (`color`, `Color`, `rgbColor`, `argbColor`, etc.),
  - reads nested color elements (`<Color>`, `<CodeColor>`) including attribute and text forms,
  - supports whitespace-separated RGB/ARGB values and numeric values with decimal formatting.

Why (one line):
- To restore correct global search placement and handle additional NVivo color encodings that were still falling back to gray.
